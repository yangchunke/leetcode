...
public class MaximumProductSubarray {
	...
	public int maxProduct(int[] A) {
		...
		for (int i = 0; i < A.length; i++) {
			if (A[i] == 0) {
				...
				if (startIdx != -1) {
					int lmax = this.localMaximum(A, startIdx, i, firstMinusIdx,
							lastMinusIdx, plus);
					if (lmax > max) {
						max = lmax;
					}
		...
		if (startIdx != -1) {
			int lmax = this.localMaximum(A, startIdx, A.length, firstMinusIdx,
					lastMinusIdx, plus);
			if (lmax > max) {
				max = lmax;
			}
	...
	private int localMaximum(int[] A, int startIdx, int endIdx,
			int firstMinusIdx, int lastMinusIdx, boolean plus) {
